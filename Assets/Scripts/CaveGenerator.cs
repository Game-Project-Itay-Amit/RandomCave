...
public class CaveGenerator {
    ...
    private int wall = 1, floor = 0, rock = 2;
    ...
    public void SmoothMap()   {
        for (int x = 0; x < gridSize; x++) {
            for (int y = 0; y < gridSize; y++) {
                ...
                int surroundingWalls = GetSurroundingWallCount(x, y, wall);
                int surroundingRocks = GetSurroundingWallCount(x, y, rock);
                ...
                // {
                //     bufferNew[x, y] = 2;//bufferOld[x, y];
                // } 
                if (surroundingWalls > 1 && surroundingWalls <= 4 || surroundingRocks > 4 ){
                    bufferNew[x, y] = 2;
                }
                else {
                    bufferNew[x, y] = 0;
    ...
    private int GetSurroundingWallCount(int cellX, int cellY, int wallType) {
        int wallCounter = 0;
        for (int neighborX = cellX - 1; neighborX <= cellX + 1; neighborX ++) {
            for (int neighborY = cellY - 1; neighborY <= cellY + 1; neighborY++) {
                ...
                if (bufferOld[neighborX, neighborY] == wallType) {
                    wallCounter += 1;
                }
            }
        }
